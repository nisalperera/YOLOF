...
class YOLOFDtasetMapper(DatasetMapper):
    ...
    def __init__(
        ...
        *,
        ...
        if not is_train:
            logger.info(f"[YoloFDatasetMapper] Constructing a YOLOF DatasetMapper for with return_val_loss={self.return_val_loss} in {mode} mode.")
        logger.info(f"[YoloFDatasetMapper] Augmentations used in {mode}: {augmentations}")
