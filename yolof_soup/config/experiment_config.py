"""
configs/experiment_config.py
============================
Single source of truth for every path, hyperparameter, and Detectron2
CfgNode factory used across the YOLOF Model-Soup thesis experiments.

Sections
--------
1.  Environment / hardware
2.  Directory layout
3.  Dataset registration & identifiers
4.  Checkpoint paths
5.  Training hyperparameters  (Phase 1 & 2)
6.  Soup-construction hyperparameters  (Phase 3)
7.  Loss-landscape hyperparameters  (Phase 4)
8.  Statistics thresholds
9.  Detectron2 CfgNode factories   (build_phase1_cfg, build_phase2_cfg,
                                    build_eval_cfg)

All paths are relative to PROJECT_ROOT so the repo is portable.
Override any value via environment variables before importing this module,
e.g.:
    export THESIS_ROOT=/scratch/nisal/thesis
"""

from __future__ import annotations

import logging
import os
from pathlib import Path

import torch

# ── Optional: pull Detectron2 lazily so this module imports even without it ──
try:
    from detectron2.config import get_cfg
    from detectron2.data import DatasetCatalog, MetadataCatalog
    _D2_AVAILABLE = True
except ImportError:
    _D2_AVAILABLE = False

logger = logging.getLogger(__name__)


# ─────────────────────────────────────────────────────────────────────────────
# 1.  Environment / Hardware
# ─────────────────────────────────────────────────────────────────────────────

#: GPU count for Detectron2 launch().  Override with env var THESIS_NUM_GPUS.
NUM_GPUS: int = int(os.environ.get("THESIS_NUM_GPUS", "1"))

#: Primary compute device for non-Detectron2 code (soup arithmetic, stats).
DEVICE: torch.device = torch.device(
    os.environ.get("THESIS_DEVICE", "cuda" if torch.cuda.is_available() else "cpu")
)

#: Mixed-precision training flag.
AMP_ENABLED: bool = DEVICE.type == "cuda"


# ─────────────────────────────────────────────────────────────────────────────
# 2.  Directory layout
# ─────────────────────────────────────────────────────────────────────────────

#: Absolute project root.  Override with env var THESIS_ROOT.
PROJECT_ROOT: Path = Path(
    os.environ.get("THESIS_ROOT", Path(__file__).resolve().parents[1])
)

#: Sub-directory tree (created on first use by each phase).
DATA_DIR:       Path = PROJECT_ROOT / "data"
CHECKPOINT_DIR: Path = PROJECT_ROOT / "checkpoints"
RESULTS_DIR:    Path = PROJECT_ROOT / "results"
LOG_DIR:        Path = PROJECT_ROOT / "logs"

# Output directories per training phase
PHASE1_OUTPUT_DIR: Path = CHECKPOINT_DIR / "phase1_backbone_encoder"
PHASE2_OUTPUT_DIR: Path = CHECKPOINT_DIR / "phase2_decoder_sweep"

# Ensure key directories exist at import time
for _d in (CHECKPOINT_DIR, RESULTS_DIR, LOG_DIR,
           PHASE1_OUTPUT_DIR, PHASE2_OUTPUT_DIR):
    _d.mkdir(parents=True, exist_ok=True)

# Convert to str for Detectron2 (which expects str, not Path)
CHECKPOINT_DIR  = str(CHECKPOINT_DIR)
RESULTS_DIR     = str(RESULTS_DIR)
LOG_DIR         = str(LOG_DIR)
PHASE1_OUTPUT_DIR = str(PHASE1_OUTPUT_DIR)
PHASE2_OUTPUT_DIR = str(PHASE2_OUTPUT_DIR)


# ─────────────────────────────────────────────────────────────────────────────
# 3.  Dataset registration & identifiers
# ─────────────────────────────────────────────────────────────────────────────

#: COCO 2017 image directories.  Override via env vars if on a cluster.
COCO_ROOT:    str = os.environ.get("COCO_ROOT",    str(DATA_DIR / "coco"))
COCO_TRAIN_IMG_DIR: str = os.path.join(COCO_ROOT, "train2017")
COCO_VAL_IMG_DIR:   str = os.path.join(COCO_ROOT, "val2017")

#: COCO annotation files.
COCO_TRAIN_ANN: str = os.path.join(COCO_ROOT, "annotations",
                                    "instances_train2017.json")
COCO_VAL_ANN:   str = os.path.join(COCO_ROOT, "annotations",
                                    "instances_val2017.json")

# ── Thesis-specific dataset splits ───────────────────────────────────────────
#
#   SELECTION_DATASET : 2 000-image subset used during greedy/CD soup search.
#                       Must be a proper subset of val2017 to avoid test leakage.
#   EVAL_DATASET      : 4 000-image held-out evaluation split.
#   TRAIN_DATASET     : full COCO train2017.
#
#   All three are registered as Detectron2 dataset names so they work
#   seamlessly with build_eval_cfg() / DatasetCatalog.

TRAIN_DATASET:     str = "coco_2017_train"
SELECTION_DATASET: str = "coco_val_selection_2000"
EVAL_DATASET:      str = "coco_val_eval_4000"

#: Annotation files for the thesis splits (generated by scripts/split_val.py).
SELECTION_ANN: str = os.path.join(COCO_ROOT, "annotations",
                                   "instances_val2017_selection2000.json")
EVAL_ANN:      str = os.path.join(COCO_ROOT, "annotations",
                                   "instances_val2017_eval4000.json")

# ── Pascal VOC 2007 (cross-domain, Phase 5) ───────────────────────────────────
#
#   VOC annotations must be pre-converted to COCO-format JSON.
#   Use:  python scripts/voc2coco.py  (or github.com/yukkyo/voc2coco)

VOC_ROOT:    str = os.environ.get("VOC_ROOT", str(DATA_DIR / "voc2007"))
VOC_IMG_DIR: str = os.path.join(VOC_ROOT, "JPEGImages")
VOC_ANN:     str = os.path.join(VOC_ROOT, "annotations",
                                 "voc2007_test_coco_format.json")
VOC_DATASET: str = "voc_2007_test"

# ── Register datasets with Detectron2 (idempotent) ───────────────────────────

def _register_datasets() -> None:
    """Register all thesis dataset splits with Detectron2's DatasetCatalog."""
    if not _D2_AVAILABLE:
        logger.warning("Detectron2 not available — dataset registration skipped.")
        return

    from detectron2.data.datasets import register_coco_instances

    _splits = [
        (TRAIN_DATASET,     COCO_TRAIN_ANN,  COCO_TRAIN_IMG_DIR),
        (SELECTION_DATASET, SELECTION_ANN,   COCO_VAL_IMG_DIR),
        (EVAL_DATASET,      EVAL_ANN,        COCO_VAL_IMG_DIR),
        (VOC_DATASET,       VOC_ANN,         VOC_IMG_DIR),
    ]

    for name, ann, img_dir in _splits:
        if name not in DatasetCatalog.list():
            try:
                register_coco_instances(name, {}, ann, img_dir)
                logger.debug("Registered dataset: %s", name)
            except FileNotFoundError:
                logger.warning(
                    "Annotation file not found for '%s': %s — "
                    "dataset will not be available until annotations exist.",
                    name, ann,
                )

_register_datasets()


# ─────────────────────────────────────────────────────────────────────────────
# 4.  Checkpoint paths
# ─────────────────────────────────────────────────────────────────────────────

#: Number of decoder ingredient runs.
N_INGREDIENTS: int = int(os.environ.get("THESIS_N_INGREDIENTS", "6"))

#: Pre-trained COCO weights to initialise Phase 1 from.
#:  • Download from: https://github.com/chensnathan/YOLOF
#:  • Place at:      checkpoints/pretrained/yolof_R_50_C5_1x.pth
PRETRAINED_WEIGHTS: str = os.path.join(
    CHECKPOINT_DIR, "pretrained", "yolof_R_50_C5_1x.pth"
)

#: Phase 1 output — backbone + encoder weights only (saved by phase1 script).
BACKBONE_ENC_CKPT: str = os.path.join(PHASE1_OUTPUT_DIR, "backbone_encoder.pth")

#: Phase 2 outputs — one decoder-only checkpoint per ingredient run.
DECODER_CKPT_PATHS: list[str] = [
    os.path.join(PHASE2_OUTPUT_DIR, f"decoder_run{i:02d}.pth")
    for i in range(N_INGREDIENTS)
]

#: Phase 2 full-model checkpoints (backbone + encoder + decoder) used
#: by Phase 4 for backbone-encoder pair analysis.
GLOBAL_CKPT_PATHS: list[str] = [
    os.path.join(PHASE2_OUTPUT_DIR, f"global_run{i:02d}.pth")
    for i in range(N_INGREDIENTS)
]

#: Best individual model checkpoint (e.g. run with highest val mAP).
#: Updated at the end of Phase 2.  Default: first ingredient.
BASELINE_CKPT: str = os.path.join(PHASE2_OUTPUT_DIR, "best_individual.pth")

#: Final soup checkpoints (written by Phase 3).
LEARNED_SOUP_CKPT: str  = os.path.join(CHECKPOINT_DIR, "learned_head_soup.pth")
GREEDY_SOUP_CKPT: str   = os.path.join(CHECKPOINT_DIR, "greedy_head_soup.pth")
UNIFORM_SOUP_CKPT: str  = os.path.join(CHECKPOINT_DIR, "uniform_head_soup.pth")
GLOBAL_SOUP_CKPT: str   = os.path.join(CHECKPOINT_DIR, "global_uniform_soup.pth")

# ─────────────────────────────────────────────────────────────────────────────
# 6.  Soup-construction hyperparameters  (Phase 3)
# ─────────────────────────────────────────────────────────────────────────────

#: λ search grid for sub-head coordinate descent.
#: Range [−0.5, 2.0] covers negative editing, identity (1.0), and upscaling.
LAMBDA_GRID: list[float] = [-0.5, 0.0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 2.0]

#: Maximum coordinate-descent passes before forced termination.
MAX_CD_PASSES: int = 20

#: Convergence tolerance — stop when max |Δλ| < this value.
CONVERGE_TOL: float = 1e-3

#: Number of images to score per soup candidate during greedy/CD selection.
#: Reducing this speeds up search at the cost of selector noise.
SELECTION_EVAL_MAX_IMGS: int = 500


# ─────────────────────────────────────────────────────────────────────────────
# 7.  Loss-landscape hyperparameters  (Phase 4)
# ─────────────────────────────────────────────────────────────────────────────

#: Number of α steps for linear interpolation in loss barrier measurement.
LMC_ALPHA_STEPS: int = 11   # α ∈ {0.0, 0.1, …, 1.0}

#: SAM perturbation radius ρ for sharpness estimation.
SAM_RHO: float = 0.05

#: Number of gradient-ascent steps in sharpness probe.
SHARPNESS_STEPS: int = 5

#: Number of images used for loss/sharpness evaluation.
LANDSCAPE_EVAL_MAX_IMGS: int = 200


# ─────────────────────────────────────────────────────────────────────────────
# 8.  Statistics thresholds
# ─────────────────────────────────────────────────────────────────────────────

#: Two-tailed α for all significance tests.
ALPHA_SIGNIFICANCE: float = 0.05

#: Bootstrap resamples for confidence intervals.
N_BOOTSTRAP: int = 10_000

#: Minimum Cohen's d to claim practical significance.
COHENS_D_THRESHOLD: float = 0.5

#: Minimum Spearman |r| to claim a meaningful correlation.
SPEARMAN_R_THRESHOLD: float = 0.3


# ─────────────────────────────────────────────────────────────────────────────
# 9.  Detectron2 CfgNode factories
# ─────────────────────────────────────────────────────────────────────────────

def _base_yolof_cfg():
    """
    Return a CfgNode pre-loaded with the canonical YOLOF-R50-C5-1x defaults.
    All factory functions call this first, then override only what they need.
    The base YAML ships with the YOLOF Detectron2 port; adjust the path to
    match your local clone.
    """
    assert _D2_AVAILABLE, "Detectron2 is required to build CfgNodes."
    cfg = get_cfg()

    # ── Pull in YOLOF's default overrides ─────────────────────────────────────
    # Adjust the path to wherever your YOLOF config YAML lives.
    _YOLOF_BASE_YAML = os.path.join(
        os.environ.get("YOLOF_CONFIG_DIR",
                       str(PROJECT_ROOT / "configs" / "YOLOF")),
        "YOLOF_R_50_C5_1x.yaml",
    )
    if os.path.isfile(_YOLOF_BASE_YAML):
        cfg.merge_from_file(_YOLOF_BASE_YAML)
    else:
        logger.warning(
            "YOLOF base YAML not found at %s — using Detectron2 defaults. "
            "Set env var YOLOF_CONFIG_DIR to fix this.", _YOLOF_BASE_YAML
        )

    # ── Hardware ──────────────────────────────────────────────────────────────
    cfg.MODEL.DEVICE           = str(DEVICE)
    cfg.SOLVER.IMS_PER_BATCH   = BATCH_SIZE_PER_GPU * max(NUM_GPUS, 1)
    cfg.DATALOADER.NUM_WORKERS = int(os.environ.get("THESIS_NUM_WORKERS", "4"))

    # ── Pretrained weights ────────────────────────────────────────────────────
    if os.path.isfile(PRETRAINED_WEIGHTS):
        cfg.MODEL.WEIGHTS = PRETRAINED_WEIGHTS
    else:
        logger.warning(
            "Pretrained weights not found at %s — "
            "training will start from scratch.", PRETRAINED_WEIGHTS
        )

    return cfg


# ── Evaluation / inference config ─────────────────────────────────────────────

def build_eval_cfg(dataset: str = EVAL_DATASET):
    """
    Lightweight CfgNode for inference-only passes (soup evaluation,
    loss landscape measurement, cross-domain evaluation).

    Parameters
    ----------
    dataset : str
        Detectron2 dataset name — one of SELECTION_DATASET, EVAL_DATASET,
        VOC_DATASET, or TRAIN_DATASET.
    """
    cfg = _base_yolof_cfg()

    cfg.DATASETS.TRAIN = ()
    cfg.DATASETS.TEST  = (dataset,)

    cfg.OUTPUT_DIR = RESULTS_DIR

    # Use batch size 1 for reproducible sequential evaluation
    cfg.SOLVER.IMS_PER_BATCH = 1

    # NMS / score threshold — keep YOLOF defaults (already in base YAML)
    # Override here if you need stricter thresholds for landscape measurement.
    # cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.05

    cfg.freeze()
    return cfg