...
LEARNED_SOUP_BATCH_SIZE: int = 32
...
def build_learned_soup(
    ...
    # alpha_raw: unconstrained logits; softmax → probability simplex
    # Initialised to zero → uniform distribution at start
    alpha_raw = torch.zeros(
        n_ingredients, device=device, dtype=torch.float32, requires_grad=True
    )

    # log_beta: unconstrained; β = exp(log_beta) ensures β > 0 always
    # Initialised to 0.0 → β = 1.0 (no temperature change at start)
    ...
    logger.info("  → Initial β (exp(log_β=0)): %.4f", torch.exp(log_beta).item())
    ...
    try:
        for epoch in range(LEARNED_SOUP_EPOCHS):
            ...
            for batch_idx, batch in enumerate(selection_dataloader):
                ...
                # Only alpha_raw and log_beta are autograd leaves.
                ...
                # Scalar proxy for the paper's objective β · f(x, Σᵢ αᵢθᵢ):
                #   weighted_loss = Σᵢ αᵢ · L̄ᵢ   (α IS in the graph)
                #   scaled_loss   = β · weighted_loss   (β IS in the graph)
                #
                # Gradients:
                #   ∂scaled_loss/∂αᵢ  = β · L̄ᵢ
                #   ∂scaled_loss/∂β   = Σᵢ αᵢ · L̄ᵢ  (via chain rule through exp)
                weighted_loss = (alpha_normalized * loss_tensor).sum()
                scaled_loss   = beta * weighted_loss
                ...
                if not scaled_loss.isfinite():
                    ...
                    del scaled_loss, weighted_loss, loss_tensor
                    continue

                # Backward is trivially cheap — only alpha_raw and log_beta are leaves
                ...
                if batch_idx > 0 and batch_idx % eval_log_freq == 0:
                    logger.debug(
                        "  Epoch %d batch %d: batch_loss=%.4f  avg=%.4f  "
                        "α=%s  β=%.4f",
                        epoch,
                        batch_idx,
                        scaled_loss.item(),
    ...
    best_beta: float = torch.exp(best_log_beta).item()

    logger.info(
        "  ✓ Optimisation complete — best α=%s  best β=%.4f",
        [f"{a:.4f}" for a in best_alpha_normalized.tolist()],
        best_beta,
    )
...
def run(verbose: bool = True, cal_bn: bool = True, force_construction: bool = False) -> Dict[str, Any]:
    ...
    Args:
        ...
        force_construction: If True, rebuild all conditions even if cached checkpoints
                            exist. If False, load existing checkpoints when found.
    ...
    try:
        ...
        logger.info("\n[1/8] Loading ingredient checkpoints...")
        ...
        # ── Build config ──────────────────────────────────────────────────────
        logger.info("\n[2/8] Building Detectron2 config...")
        cfg     = build_eval_cfg()
        ...
        if force_construction:
            logger.info("\n[4/8] Building soup conditions 1–4...")
            ...
            # ── Save checkpoints ──────────────────────────────────────────────────
            logger.info("\nSaving checkpoints...")
            save_checkpoint(checkpoint_dir / "global_uniform_soup.pth", condition_1_state)
            save_checkpoint(checkpoint_dir / "branch_uniform_soup.pth", condition_2_state)
            save_checkpoint(checkpoint_dir / "dirichlet_soup.pth", condition_3_state)
            save_checkpoint(checkpoint_dir / "fisher_weighted_soup.pth", condition_4_state)
            save_checkpoint(checkpoint_dir / "learned_soup.pth", condition_5_state)
        else:
            logger.info("\n[4/8] Loading cached soup condition checkpoints...")
            condition_1_state = load_state(checkpoint_dir / "global_uniform_soup.pth")
            condition_2_state = load_state(checkpoint_dir / "branch_uniform_soup.pth")
            condition_3_state = load_state(checkpoint_dir / "dirichlet_soup.pth")
            condition_4_state = load_state(checkpoint_dir / "fisher_weighted_soup.pth")
            condition_5_state = load_state(checkpoint_dir / "learned_soup.pth")
            logger.info("  ✓ Loaded all 5 conditions from cached checkpoints")

        # ── BN calibration ────────────────────────────────────────────────────
        if cal_bn:
            n_cal = int(train_dataset_size / train_dataloader.batch_size)
            logger.info("\n[6/8] Calibrating BN stats for all conditions...")
            for label, state in [
                ("Global Uniform",  "condition_1_state"),
                ("Branch Uniform",  "condition_2_state"),
                ("Dirichlet CD",    "condition_3_state"),
                ("Fisher-weighted", "condition_4_state"),
                ("Learned Soup",    "condition_5_state"),
            ]:
                logger.info("  Calibrating BN: %s...", label)
                calibrated = calibrate_bn(
                    cal_cfg, locals()[state], train_dataloader,
                    n_batches=n_cal, device=DEVICE
                )
                if state == "condition_1_state":   condition_1_state = calibrated
                elif state == "condition_2_state": condition_2_state = calibrated
                elif state == "condition_3_state": condition_3_state = calibrated
                elif state == "condition_4_state": condition_4_state = calibrated
                elif state == "condition_5_state": condition_5_state = calibrated
            logger.info("  ✓ BN calibration complete for all conditions")

        # ── Evaluate all 5 conditions ─────────────────────────────────────────
        logger.info("\n[7/8] Evaluating all 5 conditions...")
        ...
        map_1 = results_cond1["map50_95"]
        map_2 = results_cond2["map50_95"]
        map_3 = results_cond3["map50_95"]
        map_4 = results_cond4["map50_95"]
        map_5 = results_cond5["map50_95"]

        logger.info("\nResults summary:")
        logger.info("  Condition 1 (Global Uniform):  mAP50:95=%.4f", map_1)
        logger.info("  Condition 2 (Branch Uniform):  mAP50:95=%.4f", map_2)
        logger.info("  Condition 3 (Dirichlet CD):    mAP50:95=%.4f", map_3)
        logger.info("  Condition 4 (Fisher-weighted): mAP50:95=%.4f", map_4)
        logger.info("  Condition 5 (Learned Soup):    mAP50:95=%.4f", map_5)
        ...
        logger.info("\nCondition 1 per-class AP (sample):\n%s", table)

        # ── Determine best learned condition ──────────────────────────────────
        logger.info("\n[8/8] Determining best learned condition (M3/M4/M5)...")
        best_map = max(map_3, map_4, map_5)
        if best_map == map_5:
            best_learned_state = condition_5_state
            best_learned_cond  = 5
            best_learned_label = "Learned Soup (M5)"
        elif best_map == map_4:
            best_learned_state = condition_4_state
            best_learned_cond  = 4
            best_learned_label = "Fisher-weighted (M4)"
        else:
            best_learned_state = condition_3_state
            best_learned_cond  = 3
            best_learned_label = "Dirichlet CD (M3)"

        logger.info(
            "  → Best learned condition: %s (mAP50:95=%.4f)",
            best_learned_label, best_map,
        )

        # ── Save checkpoints ──────────────────────────────────────────────────
        logger.info("\nSaving checkpoints...")
        save_checkpoint(checkpoint_dir / "global_uniform_soup.pth", condition_1_state, 
                        metadata={"condition": 1, "method": "global_uniform",  "map50_95": map_1})
        save_checkpoint(checkpoint_dir / "branch_uniform_soup.pth", condition_2_state,
                        metadata={"condition": 2, "method": "branch_uniform",  "map50_95": map_2})
        save_checkpoint(checkpoint_dir / "dirichlet_soup.pth", condition_3_state,
                        metadata={"condition": 3, "method": "dirichlet_cd",    "map50_95": map_3})
        save_checkpoint(checkpoint_dir / "fisher_weighted_soup.pth", condition_4_state,
                        metadata={"condition": 4, "method": "fisher_weighted", "map50_95": map_4})
        save_checkpoint(
            checkpoint_dir / "learned_soup.pth", condition_5_state,
            metadata={
                "condition": 5,
                "method": "learned_soup",
                "map50_95": map_5,
        ...
        save_checkpoint(
            checkpoint_dir / "best_learned_soup.pth", best_learned_state,
            metadata={"condition": best_learned_cond, "method": best_learned_label, "map50_95": best_map},
        )

        logger.info("  ✓ global_uniform_soup.pth  → %s", checkpoint_dir)
        logger.info("  ✓ branch_uniform_soup.pth  → %s", checkpoint_dir)
        logger.info("  ✓ dirichlet_soup.pth       → %s", checkpoint_dir)
        logger.info("  ✓ fisher_weighted_soup.pth → %s", checkpoint_dir)
        logger.info("  ✓ learned_soup.pth         → %s", checkpoint_dir)
        ...
        results_summary = {
            ...
            "best_learned_condition": best_learned_cond,
            "best_learned_map50_95":  best_map,
            "hyperparameters": {
                "learned_mixing_substrings": list(LEARNED_MIXING_SUBSTRINGS),
                "learned_soup_lr":           LEARNED_SOUP_LR,
                "learned_soup_epochs":       LEARNED_SOUP_EPOCHS,
        ...
        results_path = Path(RESULTS_DIR) / "phase3_soup_results.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cal-bn",          action="store_true", default=False,
                        help="Perform BN calibration")
    parser.add_argument("--force-construction", action="store_true", default=False,
